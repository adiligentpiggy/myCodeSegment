...
static int _hashmap_hash(hmap_t in, char* key){
  ...
  /* Find the best index */
  //dzlog_debug("_hashmap_hash %u %s", m, key);
...
hmap_t hashmap_create() {
  ...
  m->size = 0;
  //dzlog_debug("hashmap_create %u", m);
  return m;
}
...
int hashmap_get(hmap_t in, const char* key, void_ptr *value){
  ...
  //dzlog_debug("hashmap_get %u %s", m, key);
  curr = _find_hash_index(m, key);
 ...
 */
int hashmap_remove(hmap_t in, const char* key, void_ptr *outValue){
  int i, curr;
  hashmap_map_t* m;
  hashmap_elem_t *elem;
  ...
  /* Find key */
  //dzlog_debug("hashmap_remove %u %s", m, key);
  curr = _find_hash_index(m, key);
